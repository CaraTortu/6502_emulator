...
fn main() {
    ...
    let test_program: [u8; 40] = [
        0xa0, 0x00, 0x84, 0x32, 0xb1, 0x30, 0xaa, 0xc8, 0xca, 0xb1, 0x30, 0xc8, 0xd1, 0x30, 0x90,
        0x10, 0xf0, 0x0e, 0x48, 0xb1, 0x30, 0x88, 0x91, 0x30, 0x68, 0xc8, 0x91, 0x30, 0xa9, 0xff,
    ...
    while byte < test_program.len() - 1 {
        ...
        let param: Option<isize> = match OPCodes::instruction_to_opcode(
            test_program[byte],
            Some(test_program[byte + 1] as isize),
        ) {
            CLC | CLD | CLI | CLV | SEC | SED | SEI | PHP | PHA | PLP | PLA | RTI | RTS | TAX
            | TAY | TSX | TXA | TXS | TYA | DEX | DEY | INX | INY | BRK | NOP => {
